...
import { isDefined, isNotNull } from "../../../types/TypeGuard";
...
export default class GameState implements GameStateDto {
  ...
  public static isInBoardGameBounds(
    board: GameState["board"],
    row: number,
    col: number
  ) {
    if (board.length === 0 || board[0]!.length === 0) {
      throw new Error("Le plateau de jeu est vide.");
    }
    return row >= 0 && row < board.length && col >= 0 && col < board[0]!.length;
  }
  public static isCellOccupied(
    board: GameState["board"],
    row: number,
    col: number
  ) {
    if (!this.isInBoardGameBounds(board, row, col)) {
      throw new Error(
        `Les coordonnées (${row}, ${col}) sont en dehors des limites du plateau de jeu.`
      );
    }
