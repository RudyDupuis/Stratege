...
<template>
  <section class="flex flex-col justify-center items-center size-full">
    ...
    <p
      v-if="playersInfo.some((playerInfo) => !playerInfo.isConnected)"
      class="mb-10"
    >
    ...
    <section class="flex justify-center space-x-5 md:space-x-10 mb-10">
      ...
      >
        <PawnDisplay
          size-class="size-5"
          :color-class="
            playerInfo.role === PlayerRole.Player1 ? 'bg-player1' : 'bg-player2'
          "
        ...
        <p>
          ...
          <span :class="playerInfo.isConnected ? 'text-success' : 'text-error'">
            ●
          </span>
        </p>
        ...
        <template v-else>
          <div
            ...
            "
            class="flex bg-dark-light p-5 rounded-xl flex-col items-center mt-5"
          >
            <AvatarFinder
              :avatar-id="
                usersLinkedToConnectedPlayers[playerInfo.userId]!.avatarId
              "
              class="w-16 h-16"
            />
